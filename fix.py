...
class Worker_fix(Agent):
    ...
    def new_policy(self):
        # The gap has to be reduced not by flattering the conditions but by creating ad hoc solutions.
        # Support for families: kinder garden, flexible timetables, work from homw, babysitting bonuses.
        ...
        if self.gender == 'F' and self.age <= 35 and self.level != 0:
            ...
            # Moreover, companies could improve training programs top down to increase on one side the opportunities for females 
            # employees and more in general the human capital value across the company
...
class LabourMarket_fix(Model):
    def __init__(self, N, width, height, M=0.4):
        ...
        for i in range(self.num_agents):
            ...
            a = Worker_fix(i, self, level, age, education,total_tenure)
            self.schedule.add(a)
    ...
    def add_agents(self):
        ...
        if new_entry > 0:        
            for i in range(new_entry):
                ...
                a = Worker_fix(self.total_agents_count, self)
                self.schedule.add(a)
